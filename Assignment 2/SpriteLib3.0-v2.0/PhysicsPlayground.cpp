...
void PhysicsPlayground::InitScene(float windowWidth, float windowHeight)
{
	...
	//Setup static Wall
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(310.f), float32(-75.f));
	...
	//stationary box
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(330.f), float32(-85.f));
	...
	//Setup static Wall
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(450.f), float32(-75.f));
	...
	//Setup static Platform
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(525.f), float32(-50.f));
	...
	//Setup static Wall
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(600.f), float32(-10.f));
	...
	//stationary box
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(585.f), float32(-35.f));
	...
	//stationary box
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(565.f), float32(-35.f));
	...
	//Setup static RAMP
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(530.f), float32(-35.3f));
	...
	//ball
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_dynamicBody;
		tempDef.position.Set(float32(500.f), float32(-35.f));

		tempBody = m_physicsWorld->CreateBody(&tempDef);

		tempPhsBody = PhysicsBody(entity, tempBody, (float)((tempSpr.GetWidth() - shrinkX) / 2.0f), vec2(0.f, 0.f), false, GROUND, GROUND | ENVIRONMENT | PLAYER | TRIGGER, 0.3f);
		tempPhsBody.SetColor(vec4(0.f, 1.f, 0.f, 0.3f));
		tempPhsBody.SetRotationAngleDeg(90.f);
	}
